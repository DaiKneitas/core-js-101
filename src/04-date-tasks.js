...
function timeSpanToString(startDate, endDate) {
  ...
  sec = sec.toString();
  let milliSec = Math.floor(diff % 1000);
  ...
  if (milliSec.length < 3) {
    milliSec = '0'.repeat(3 - milliSec.length) + milliSec;
  }
...
function angleBetweenClockHands(date) {
  ...
  const hoursAng = (hours % 12) * 30 + minutes * 0.5;
  const minutesAng = minutes * 6;
  const ang = Math.abs(hoursAng - minutesAng);
  return Math.min(ang, 360 - ang) * (Math.PI / 180);
}
